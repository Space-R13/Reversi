...
//    boolean player;
 //   int x = 0, y = 0;
/**
 * Делая ход, игрок должен поставить свой бочонок между уже находящейся на доске фишкой своего цвета и непрерывной линией (горизонтальной, вертикальной или по диагонали)
 * бочонков соперника, как бы закрывая фишки соперника с двух сторон.
 * Количество перекрытых фишек соперника не важно - чем больше, тем лучше. При этом можно за один ход закрывать несколько таких линий.
 *
 *

 *
 *  Проанализировать массив по координатам и определить где стоят фишки игроков(циклом for).
 *  Определил координаты black. if black тогда мы смотрим отдельно в каждую сторону и проверяем заполнение точек в массиве.
 * Если точка пустая, то хода нет, если занята своей бочкой, то хода нет, если занята фишкой соперника, то проверяем клетки относительно фишки соперника по траектории,
 * если своя фишка , то хода нет, если точка пустая, то предлагается ход на пустую клетку, если точка занята бочкой соперника, то повторяем проверку следующей точки по траектории.
 *
 *
 *  - For(){
 *      if
